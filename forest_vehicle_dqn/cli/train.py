...
def _forest_choose_replacement_candidate(
    env: AMRBicycleEnv,
    q: torch.Tensor,
    *,
    candidates: list[int],
    ...
    for a_id in cand_list:
        ...
        od, coll = env._od_and_collision_at_pose_m(x1, y1, psi1)
        if bool(coll):
            continue

        dist1 = float(env._progress_dist_pose_m(x1, y1, psi1))
        if not math.isfinite(dist1):
            continue

        od1 = float(od) if math.isfinite(float(od)) else -float("inf")
        q_score = float(q[a_i].item())
        if float(penalty_w) > 0.0:
            turn_cost = abs(float(delta1)) / float(delta_max) + 0.5 * abs(float(delta_dot)) / float(delta_dot_max)
            q_score -= float(penalty_w) * float(turn_cost)
...
def _forest_policy_action_from_q(
    ...
    *,
    ...
    if (not bool(strict_no_fallback)) and (not bool(a0_adm)):
        ...
        if admissible_topk:
            chosen = _forest_choose_replacement_candidate(
                env,
                q,
...
def train_one(
    ...
    *,
    ...
    log(
        ...
        f"forest_min_progress_m={float(min_prog_m):.4f}, "
        f"forest_topk_turn_penalty={float(topk_turn_penalty):.3f}, "
        f"strict_no_fallback={bool(strict_no_fallback)}"
    )
...
def build_parser() -> argparse.ArgumentParser:
    ...
    ap.add_argument(
        ...
        choices=("euclid", "grid4", "dijkstra8_nocorner"),
        default="euclid",
        help=(
    ...
    ap.add_argument(
        ...
        default=2.0,
        help=(
            "Forest-only: for progress_dist_mode=dijkstra8_nocorner, clearance penalty weight (>=0). "
            "Higher => prefer paths with larger obstacle clearance."
        ),
    )
    ap.add_argument(
        ...
        default=0.5,
        help=(
            "Forest-only: for progress_dist_mode=dijkstra8_nocorner, clearance penalty decay length (meters, >0). "
            "Larger => penalty decays more slowly with clearance."
        ),
    )
    ap.add_argument(
    ...
    for env_name in args.envs:
        ...
        if env_name in FOREST_ENV_ORDER:
            env = AMRBicycleEnv(
                ...
                progress_cost_w_clearance=float(args.forest_progress_cost_w_clearance),
                progress_cost_sigma_m=float(args.forest_progress_cost_sigma_m),
            )
