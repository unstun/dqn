...
| V8 迭代入口（候选） | `configs/v8p10.json` | progress-dist clearance 消融（`dijkstra8_nocorner` + `progress_cost_w_clearance` sweep；目标：C 门槛） | infer-only sweep smoke 已跑（fixed pairs3，runs=3；当前 best：w=2.0；baseline gap）→ full gate 暂不建议 |
