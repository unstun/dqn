...
| V8 迭代入口（候选） | `configs/v8p16.json` | 训练侧：观测加入 progress distance-field map（occ + dist），目标：long detour 明显回落并压过 baseline（目标：C 门槛） | smoke 已跑（fixed pairs3）：SR=1.0 但 short/long 均落后 baseline → NO-GO |
