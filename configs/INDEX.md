...
| V8 迭代入口 | `configs/v8p6.json` | replace-topq（替换候选 Top-Q 约束：把 tie-break 限制在高 Q 小集合内） | infer-only smoke 通过（topq=1/2/3）；train+infer smoke NO-GO（short/mid collision=1/3） |
