...
- `v7p2p2` attempted an epsilon-decay fix (`eps_decay: 4500 -> 200`) and passed mechanism checks, but smoke (`train 150 + infer runs=3`) still showed no stable gain.
- Mainline remains `v7p1` behavior (forest bicycle observation `10 + N*N`) for continued iteration.
- Failure archives: `docs/versions/v7p2p1/`, `docs/versions/v7p2p2/`.
...
> 说明：本索引用于统一 `docs/versions/` 的重编号口径；历史目录 `v3p1`~`v3p11` 保留原记录，未纳入本轮重编号；早期误混入版本链已于 2026-02-09 清理，当前主线编号延续至 `v7p2p2`。
...
| `v7p2p2` | `docs/versions/v7p2p2/` | `configs/v7p2p2.json` | `runs/v7p2p2_smoke150/train_20260220_230753/infer/20260220_232053` | `0.667 / 0.333` | `1.00 / 1.00` | smoke 失败归档，主线保持 `v7p1` |
...
- Prepare next iteration as `<version+1>` (example: `v7p2p3`).
