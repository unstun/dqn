...
- 当前 V8 迭代入口为 `v8p6`（`configs/v8p6.json`）（infer-only smoke PASS：topq=1/2/3；推荐 topq=3；train+infer smoke 未跑）。
...
# v8p6：train+infer smoke（episodes=150, runs=3）[建议先通过 infer-only smoke]
