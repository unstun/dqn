...
- 当前 V8 迭代入口为 `v8p7`（`configs/v8p7.json`）（infer-only smoke PASS：接近目标速度整形；fixed v8p6 checkpoint；SR=1.0；mid/long 的 path_time_s 仍落后 baseline；train+infer smoke 待跑）。
