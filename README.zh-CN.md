...
- 当前 V8 迭代入口为 `v8p2`（`configs/v8p2.json`）（costmap dijkstra progress distance；待 smoke）。
...
当前 V8 smoke profiles（costmap dijkstra progress distance，实验性）：

```bash
# infer-only A/B（固定 v7p1 checkpoint）：dijkstra8_nocorner vs euclid
