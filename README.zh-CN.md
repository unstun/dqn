...
- `v7p2p2` 进行过 ε 衰减修复（`eps_decay: 4500 -> 200`），机制生效，但 smoke（train150 + infer runs=3）未体现稳定收益。
- 主线继续保持 `v7p1` 行为口径（forest bicycle 观测维度为 `10 + N*N`），用于继续迭代。
- 失败留档见：`docs/versions/v7p2p1/`、`docs/versions/v7p2p2/`。
