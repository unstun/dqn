...
说明：`v6p5` 保留 epsilon 自适应衰减（`eps_decay=0`, `eps_decay_ratio=0.67`），切换为局部高分辨率观测（`obs_map_mode=local`, `obs_map_size=64`, `obs_local_range_m=18.0`），并在 `CNNQNetwork` 卷积层启用 `BatchNorm2d`。

GPU 优先 profile（同方法族，增加 CUDA 侧工作负载）：
