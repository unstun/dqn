# v7p2p6 结果对比

## 数据来源
- KPI（均值）：`runs/v7p2p6_foundationfix_smoke/train_20260221_211603/infer/20260221_213248/table2_kpis_mean_raw.csv`
- KPI（逐回合）：`runs/v7p2p6_foundationfix_smoke/train_20260221_211603/infer/20260221_213248/table2_kpis_raw.csv`
- 运行口径：`episodes=150`（train smoke）+ `runs=3`（infer short/mid/long）。

## 代码级验证结果

### 1) 本地单测
- 命令：
  - `conda run -n ros2py310 python -m pytest tests/test_agent_training_controls.py tests/test_globalcnn_network.py -v`
- 结果：`8 passed`
- 覆盖点：
  - 奖励缩放/裁剪入 replay；
  - 梯度裁剪观测指标回传；
  - 预训练梯度统计记录；
  - GlobalCNN 结构/roundtrip/旧 checkpoint 兼容。

### 2) self-check
- 命令：
  - `conda run -n ros2py310 python train.py --self-check`
  - `conda run -n ros2py310 python infer.py --self-check`
- 结果：通过（CUDA 可用，`device_ok=cuda:0`）。

## 训练日志机制验收（关键摘录）
- `train_flow.log` 显示：
  - `reward_scale=0.1000, reward_clip_abs=10.0000, grad_clip_norm=5.0000`
  - `Demo pretrain hold: early-stop gated by min_effective_steps=8000`
  - `Demo pretrain done: steps=8000/30000`
  - `RL progress ... grad_norm_ema=..., grad_clip_hit_rate=1.000`

## short/mid/long 指标（CNN vs Hybrid A*-MPC）

| 套件 | 算法 | success_rate | avg_path_length | path_time_s | avg_curvature_1_m |
|---|---|---:|---:|---:|---:|
| short | CNN-DDQN | 1.000 | 20.5613 | 18.2667 | 0.166511 |
| short | Hybrid A*-MPC | 1.000 | 17.0342 | 10.2667 | 0.114272 |
| mid | CNN-DDQN | 0.667 | 34.4835 | 36.6500 | 0.307094 |
| mid | Hybrid A*-MPC | 1.000 | 24.0814 | 13.3333 | 0.058072 |
| long | CNN-DDQN | 0.000 | N/A | N/A | N/A |
| long | Hybrid A*-MPC | 1.000 | 43.0107 | 22.8167 | 0.068718 |

## 门槛检查（三条不等式）

| 套件 | `SR(CNN)>=SR(Hybrid)` | `Path(CNN)<Path(Hybrid)` | `Time(CNN)<Time(Hybrid)` | 结论 |
|---|---|---|---|---|
| short | 通过 | 不通过 | 不通过 | 未通过 |
| mid | 不通过 | 不通过 | 不通过 | 未通过 |
| long | 不通过 | 不通过 | 不通过 | 未通过 |

## 相对 `v7p2p5` 的趋势对照（仅作 smoke 门参考）
- `v7p2p5` CNN：short/mid/long `SR=0.333/0.667/0.667`，`path_time_s=17.150/28.150/51.000`。
- `v7p2p6` CNN：short/mid/long `SR=1.000/0.667/0.000`，`path_time_s=18.267/36.650/N/A`。
- 结论：short 提升明显，但 long 退化到 `SR=0.0`，整体仍不满足 go 条件。

## 最终门槛口径状态（short/long 各 runs=20）
- short（runs=20）：`N/A`（本轮仅 smoke）。
- long（runs=20）：`N/A`（本轮仅 smoke）。
- 结论：本轮未进入最终门槛评测。

## `failure_reason` 分布（来自 `table2_kpis_raw.csv`）
- CNN-DDQN：
  - short：`reached=3`
  - mid：`reached=2`，`timeout=1`
  - long：`timeout=3`
  - 合计：`reached=5`，`timeout=4`
- Hybrid A*-MPC：
  - short：`reached=3`
  - mid：`reached=3`
  - long：`reached=3`
  - 合计：`reached=9`

## smoke 门结论（go/no-go）
- 结论：**NO-GO**。
- 原因：虽然 short 成功率提升，但 long 套件完全失效（`SR=0.0`），且 path/time 仍不占优。
- 处理：`v7p2p6` 失败归档，保持当前代码继续前向迭代，不执行代码回退。
